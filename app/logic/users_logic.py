...
def register_user_logic(email_user, password_user=None):
    ...
    """
    if password_user:
        # Si hay una contraseña proporcionada (cuando el registro es manual)
        hashed_password = generate_password_hash(password_user)
    else:
        # Si no se proporciona una contraseña (cuando es un registro vía Auth0), asignar un valor temporal o None
        hashed_password = None

    user_data = {
        'email': email_user,
        'password': hashed_password,  # Almacena la contraseña hasheada
        'tasks': []  # Inicializa el campo tasks como una lista vacía
    }
...
def auth_callback_logic(session):
    ...
    try:
        ...
        if not token:
            return {'message': 'No se pudo obtener el token de Auth0.', 'status': 'error', 'redirect_url': url_for('auth.auth_callback')}

        # Obtener la información del usuario
        user_info = oauth.parse_id_token(token)
        if not user_info:
            return {'message': 'Error al procesar el token del usuario.', 'status': 'error', 'redirect_url': url_for('auth.auth_callback')}
        ...
        return {'message': f'Error: {str(e)}', 'status': 'error', 'redirect_url': url_for('auth.auth_callback')}
